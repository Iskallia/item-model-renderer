...
export const {
	...
	const queueItemForRender = (ctx: RenderContext) => {
		...
		const mcmetaFetchTasks = ObjUtils.getValues(ctx.itemModel.textures).map(async (textureId) => {
			return [textureId, await props.resolveMcmeta(textureId)] as const;
		});

		Promise.all(mcmetaFetchTasks)
			.then((result) => {
				result.forEach(([textureId, mcmeta]) => cachedMcmeta.current.set(textureId, mcmeta));
				mcmetaQueue.current.delete(ctx.itemId);

				renderContextMap.current.set(ctx.itemId, ctx);

				setRenderingItem((renderingItem) => {
					const hasOverhead = renderingItem != null || renderQueue.current.size > 0;
